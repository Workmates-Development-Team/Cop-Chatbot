...
import { X, Send, MessageCircle, Shield, Car, Home, Globe, AlertTriangle, TrendingUp, User, Copy, Edit2 } from 'lucide-react';
...
const Chatbot = () => {
  ...
  // Clean Markdown by removing unwanted syntax (e.g., ** for bold) - optional with remark-gfm
  const cleanMarkdown = (text: string) => {
    return text; // remark-gfm handles bold and other markdown, so cleaning is optional
  };
  ...
  // Save edited message and re-send to API
  const handleInlineEditSave = async (index: number) => {
    if (editText.trim() === '') return;
    // Update the message in state
    setSectionMessages(prev => {
      const msgs = [...(prev[currentCategory] || [])];
      if (msgs[index]) {
        msgs[index] = { ...msgs[index], text: editText };
      }
      return { ...prev, [currentCategory]: msgs };
    });
    setEditIndex(null);
    setEditText('');

    // Remove all bot messages after this user message (if any)
    setSectionMessages(prev => {
      const msgs = [...(prev[currentCategory] || [])];
      const nextBotIndex = msgs.findIndex((msg, idx) => idx > index && !msg.isUser);
      const cutIndex = nextBotIndex !== -1 ? nextBotIndex : msgs.length;
      return {
        ...prev,
        [currentCategory]: msgs.slice(0, cutIndex)
      };
    });

    // Re-send the edited prompt to the API
    setIsLoading(true);
    try {
      const question =
        currentCategory === 'general'
          ? editText
          : `${editText} (category: ${currentCategory})`;
              ...
              <div className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                <div className="flex-1 px-4 pt-2 pb-4 overflow-y-auto space-y-3">
                  {messages.map((message, index) => (
                    <div key={index} className={`flex flex-col items-end ${message.isUser ? '' : 'items-start'}`}>
                      <div
                        className={`max-w-[80%] p-3 rounded-lg ${
                          message.isUser ? 'bg-blue-600 text-white self-end' : 'bg-gray-50 text-gray-800 self-start'
                        }`}
                      >
                        {message.isUser && editIndex === index ? (
                          <input
                            value={editText}
                            onChange={e => setEditText(e.target.value)}
                            className="w-full bg-blue-600 text-white border-none outline-none placeholder-white"
                            autoFocus
                            onKeyDown={e => {
                      ...
                      {message.isUser && (
                        <div className="flex gap-2 mt-1 mb-2">
                          {editIndex === index ? (
                            ...
                            <>
                              <button
                                className="p-1 rounded hover:bg-blue-700/20"
