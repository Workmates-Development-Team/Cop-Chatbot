...
const Chatbot = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch('https://d7dk2pxyrxbfuzvtwgmnsjy6xa0trbdr.lambda-url.us-east-1.on.aws/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const botResponse = data.document || 'No response received from the server.';
